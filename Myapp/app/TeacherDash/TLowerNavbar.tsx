...
import TDeliveryManDashboard from './TDeliveryManDashboard';
...
import TeacherScreen2 from './TeacherScreen2';
...
const Navbar = ({ onTabChange }) => {
  ...
  const renderContent = () => {
    switch (selectedTab) {
      case 'home':
        return <TDeliveryManDashboard key={refreshKey} isRefreshing={isRefreshing} onRefresh={refreshData} />;
      case 'profile':
      ...
      case 'analytics':
        return <TeacherScreen2 key={refreshKey} isRefreshing={isRefreshing} onRefresh={refreshData} />;
      default:
        return null;
    }
  };
  ...
  const tabs = [
    ...
    { name: 'Add', icon: 'plus-circle', label: 'AddT' },
    { name: 'analytics', icon: 'chart-bar', label: 'Techer' },
